...
// sick.h includes a header on Windows that defines an interfering macro
#undef SendMessage

DEFINE_string(id, "sick0", "Vehicle-wide unique ID used to identify this client to the server");
...
class Client final : public whisker::Init::Context {
  ...
  private:
    void InitContext(Json::Value&& config) override {
        ...
        auto sick = std::make_shared<Sick>(vehicle_config["clients"][FLAGS_id]);
        ...
        *init_msg.mutable_lidar_properties() = sick->GetSensorProperties();
        ...
        event_handlers.SetMessageHandler<whisker::proto::RequestObservationMessage>(
                [sick](auto&& message, auto& connection) { connection.SendMessage(sick->GetLatestObservation()); });
